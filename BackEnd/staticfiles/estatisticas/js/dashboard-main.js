...
function invalidateCache() {
  if (window.dashboardMain) {
    window.dashboardMain.showToast("Cache invalidado com sucesso!", "success");
  }
}

function toggleChartType(chartId) {
  if (window.dashboardMain) {
    window.dashboardMain.showToast("Alternando tipo de gráfico...", "info");
  }
}

function updateTimelineChart(period) {
  if (window.dashboardMain) {
    window.dashboardMain.showToast(`Atualizando para período: ${period}`, "info");
  }
}

function updateRanking(criteria) {
  if (window.dashboardMain) {
    window.dashboardMain.showToast(`Ranking atualizado por: ${criteria}`, "info");
  }
}

function updateComparison() {
  if (window.dashboardMain) {
    window.dashboardMain.showToast("Comparação atualizada!", "info");
  }
}
